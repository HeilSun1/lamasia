...
**看门狗**：另有独立任务 `LaMasia_Local_Watchdog` 每 4 小时检查一次，专门抓
「进程被杀/休眠导致更新根本没跑完」这类主脚本自己也发现不了的情况
（挂死的进程连一行日志都不会留）。日志 `scripts/watchdog.log`。
若发现任务已超过 12 小时没跑，会自动补跑一次（每 6 小时最多一次）。
