/* ═══════════════════════════════════════════════════════════════
   ...
   ─────────────────────────────────────────────────────────────
   行为：
     1. 页面打开先渲染本地缓存 window.DQD_U16_CACHE（每日脚本生成，保证离线可看）
...
(function () {
  ...
  function normalizeLive(playersJson, lastJson, nextJson) {
    const players = ((playersJson && playersJson.players) || []).map(function (p) {
      ...
      return {
        ...
        photo: (window.ManualPhoto && window.ManualPhoto("u16:" + id)) || "https://img.sofascore.com/api/v1/player/" + pr.id + "/image",
  ...
  function renderInjuries(data) {
    ...
    injured.forEach(function (p) {
      ...
      if (window.ManualPhoto) { const m = window.ManualPhoto("u16:" + p.id); if (m) p.photo = m; }
  ...
  function renderRoster(data) {
    ...
    const coach = (data && data.coach) || (window.DQD_U16_CACHE && window.DQD_U16_CACHE.coach) || null;
    if (coach && coach.name) {
      const c = coach;
      if (window.ManualPhoto) { const m = window.ManualPhoto("u16:coach-" + c.id); if (m) c.photo = m; }
    ...
    POS_ORDER.forEach(function (code) {
      ...
      ps.forEach(function (p) {
        ...
        const teamTag = p.team && p.team !== "Barcelona U16"
          ? '<span class="u16-team-tag" title="Sofascore 当前所属球队">' + esc(p.team.replace("Barcelona ", "巴萨")) + "</span>" : "";
        const avatar = '<span class="pl-avatar">' +
          (p.photo ? '<img src="' + esc(p.photo) + '" alt="' + esc(display) + '" loading="lazy" referrerpolicy="no-referrer"' +
        ...
        html +=
          '<div class="pl-row">' +
            ...
            avatar +
            '<span class="pl-name" data-player-key="sf:u16:' + esc(p.id) + '" title="点击查看球员卡片"><span class="zh">' + esc(display) + injBadge + teamTag + "</span>" +
