# ═══════════════════════════════════════════════════════════════
#   拉玛西亚信息站 · 巴萨U16（Cadete A）球队资讯每日更新脚本
#
#   每天从懂球帝抓取巴萨U16的「球队资讯」，生成本地缓存供页面展示：
#     1. GET 懂球帝 JSON API（team_id=50080323）
#     2. 解析 articles：标题 / 时间 / 分类 / 缩略图 / 原文链接
#     3. 生成缓存 assets/js/dqd-u16-news-cache.js
#
#   由本机计划任务 / GitHub Actions 每日调用。
#   运行日志：scripts/u16-news-update.log
...
$ApiUrl      = "https://api.dongqiudi.com/v3/archive/app/channel/feeds?id=50080323&type=team&size=30&platform=web&version="
$UserAgent   = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0 Safari/537.36"
$ArticleBase = "https://www.dongqiudi.com"
$MaxNews     = 50                                        # 滚动存档上限（新+旧按文章 id 去重后保留）

$Root      = Split-Path -Parent $PSScriptRoot
$CacheFile = Join-Path $Root "assets\js\dqd-u16-news-cache.js"
$LogFile   = Join-Path $Root "scripts\u16-news-update.log"
...
if (Test-Path $CacheFile) {
  try {
    ...
    $m = [regex]::Match($txt, 'window\.DQD_U16_NEWS\s*=\s*(\{.*\})\s*;', [System.Text.RegularExpressions.RegexOptions]::Singleline)
    if ($m.Success) {
...
[void]$sb.AppendLine("/* 自动生成，请勿手动编辑 —— 由 update_u16_news.ps1 每日更新于 $updated 数据源：懂球帝 */")
[void]$sb.AppendLine("window.DQD_U16_NEWS = {")
