/* ═══════════════════════════════════════════════════════════════
   拉玛西亚信息站 · 巴萨U18（Juvenil B）球队资讯渲染
   ─────────────────────────────────────────────────────────────
   数据来自懂球帝「巴塞罗那U18」球队资讯页，由 update_u18_news.ps1
   每日生成缓存。懂球帝页面未开放 CORS，前端只渲染每日缓存。
   缓存为滚动存档（上限 50 条）：初始展示最近 8 条，点「查看更多」展开。
   仅用于 teams/juvenil-b.html
   ═══════════════════════════════════════════════════════════════ */
(function () {
  "use strict";

  const el = document.getElementById("u18-news");
  if (!el) return;
  const statusEl = document.getElementById("u18-news-status");
  ...
  const data = window.DQD_U18_NEWS;
