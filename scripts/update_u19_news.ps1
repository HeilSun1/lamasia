# ═══════════════════════════════════════════════════════════════
#   拉玛西亚信息站 · 巴萨U19（Juvenil A）球队资讯每日更新脚本
#
#   每天从懂球帝抓取巴萨U19的「球队资讯」，生成本地缓存供页面展示：
#     1. GET https://pc.dongqiudi.com/team/11815（球队资讯 SSR 页面）
#     2. 解析 tp-news-item 条目：标题 / 时间 / 分类 / 缩略图 / 原文链接
#     3. 生成缓存 assets/js/dqd-u19-news-cache.js
#
#   由 GitHub Actions 每日调用（或本机计划任务）。
#   运行日志：scripts/u19-news-update.log
...
$PageUrl     = "https://pc.dongqiudi.com/team/11815"      # 巴塞罗那U19 球队资讯页
$Referer     = "https://pc.dongqiudi.com/team/11815"
$UserAgent   = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0 Safari/537.36"
$ArticleBase = "https://www.dongqiudi.com"
$MaxNews     = 50                                        # 滚动存档上限（新+旧按文章 id 去重后保留）

$Root      = Split-Path -Parent $PSScriptRoot
$CacheFile = Join-Path $Root "assets\js\dqd-u19-news-cache.js"
$LogFile   = Join-Path $Root "scripts\u19-news-update.log"
...
if (Test-Path $CacheFile) {
  try {
    ...
    $m = [regex]::Match($txt, 'window\.DQD_U19_NEWS\s*=\s*(\{.*\})\s*;', [System.Text.RegularExpressions.RegexOptions]::Singleline)
    if ($m.Success) {
...
[void]$sb.AppendLine("/* 自动生成，请勿手动编辑 —— 由 update_u19_news.ps1 每日更新于 $updated 数据源：懂球帝 */")
[void]$sb.AppendLine("window.DQD_U19_NEWS = {")
