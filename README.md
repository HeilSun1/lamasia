...
本机任务每天 09:00 / 15:00 / 21:00 跑一轮。脚本自己会处理常见的「卡死」情况，
处理不了或需要你介入的会主动通知。

**能自动修好的**（不需要你管）：

- 上一轮被中断/杀掉，在工作区留下冲突标记 → 缓存类文件以远端为准自动消解
- 进程被杀留下的陈旧 `index.lock`、残留的 rebase/merge 状态 → 自动清理
- 推送被远端新提交挡住 → 重新 rebase 后再推

**会通知你的**（两种情况都会弹 Windows 通知 + 网站顶部横幅）：

- 手写源码文件（`videos-ui.js` 等）发生冲突：脚本**绝不会覆盖你的代码**，
  会将改动完整存入 stash，然后提前结束本轮。处理方式：

  ```bash
  git stash list        # 看有哪些
  git stash pop         # 恢复（有冲突就手动合并）
  ```

- 推送失败、核心数据源没刷新等。看日志：`scripts/local-daily-update.log`

**为什么网站上也显示提醒**：机器关机/休眠时，Windows 通知不可能弹出来，计划任务
也跑不起来。网站横幅是持久通道（任何人打开站点都能看到），通知是即时通道。两者互补。

**看门狗 —— 2026-09-22 已注销停用。** 曾经有一个独立任务 `LaMasia_Local_Watchdog`
每 4 小时检查一次，抓「进程被杀/休眠导致更新根本没跑完」这类主脚本自己也发现不了的情况。
**现已删除**，原因是它 130 轮运行里**一次都没有真的修复过东西**，只产出过假警报
（`SCHED_S_TASK_RUNNING=267009`「任务正在运行中」被当成失败、以及 `2147946720`），
而它的两项设计主张都已被别处覆盖：

- 「超过 12 小时没跑就自动补跑」→ 计划任务的 `StartWhenAvailable` 已经负责
  （机器不在而错过，回来会自动补跑）；
- 「发现进程被杀留下残留」→ 每轮更新开头的 `Clear-StaleGitState` 自愈已经负责。

另外它和主脚本同机同调度，机器关着时自己也一起哑掉，独有价值窗口很窄；
而每次多余的补跑都会多打一次 fcbarcelona.es（官网 2026-09-21 起有抓取风控）。

**脚本文件仍保留在库里**（`scripts/run_daily_watchdog.ps1` + `register_watchdog_task.ps1`），
想恢复跑一次注册脚本即可。停用后提醒只剩两条通道：主脚本的 `Send-LamAsiaAlert`
（真 blocked 才弹、12 小时去重）+ 下面说的网站横幅。

**退出码**：`0` 正常 / `2` 降级（部分数据源失败，站点仍在更新）/
`1` 需人工介入。计划任务的「上次运行结果」会反映这个值。

**自测**（不碰真实仓库与线上站点，在临时目录跑故障注入）：

```bash
powershell -File scripts/selftest/e2e.ps1           # 每日更新的故障注入
node scripts/selftest/status-banner.test.js        # 横幅判定逻辑
node scripts/selftest/status-banner-dom.test.js    # 横幅注入路径
```
